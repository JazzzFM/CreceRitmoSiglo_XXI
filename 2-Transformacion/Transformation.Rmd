...
```{r arrange21, echo=TRUE}
bd <- tibble(x = c(5, 7, NA, 3, 5, 2, NA))
arrange(bd, x)
arrange(bd, desc(x))
```

1 ¿Cómo usarías la función `arrange()` para ordenar todos los valores de empezando por los NA? (Tip: utiliza `is.na()`)

```{r ejercicio9, exercise=TRUE}
arrange(bd, desc(is.na(_)))
```
...
**Hint:** Usa la función `desc()` en la variable necesaria. 
</div>
...
```{r ejercicio12, exercise=TRUE}
arrange(flights, ___)
```

<div id="ejercicio12-hint">
**Hint:** Usa la función `desc()` en la variable necesaria. 
</div>

```{r ejercicio12-solution}
