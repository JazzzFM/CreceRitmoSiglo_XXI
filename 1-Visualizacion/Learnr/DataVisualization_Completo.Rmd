...
Es posible colorear una gráfica de barras de dos maneras con los parámetros color y fill.
...
```{r ajuste3, warning=FALSE}
ggplot(data = diamonds) +
  geom_bar(mapping = aes(x = cut, fill = clarity))
```

```{r ajuste4, warning=FALSE}
ggplot(data = diamonds, mapping = aes(x = cut, fill = clarity)) +
  geom_bar(alpha = 1/5, position = "identity")
```

```{r ajuste5, warning=FALSE}
ggplot(data = diamonds, mapping = aes(x = cut, colour = clarity)) +
  geom_bar(fill = NA, position = "identity")
```

La posición identidad es más útil en geoms de dos dimensiones ( es el valor por defecto)

```{r ajuste51, warning=FALSE}
ggplot(data = diamonds) +
  geom_bar(mapping = aes(x = cut, fill = clarity), position = "fill")
```

```{r ajuste6, warning=FALSE}
ggplot(data = diamonds) +
  geom_bar(mapping = aes(x = cut, fill = clarity), position = "dodge")
```

Hay una posición que funciona para gráficas de dispersión y no para gráficas de barras en este gráfica sólo se muestran 126 puntos a pesar de que existen 234 (porque se enciman)

```{r ajuste7, warning=FALSE}
ggplot(data = mpg) +
  geom_point(mapping = aes(x = displ, y = hwy))
```
...
```{r ajuste8, warning=FALSE}
ggplot(data = mpg) +
  geom_point(mapping = aes(x = displ, y = hwy), position = "jitter")
```

Agregar esta aleatoriedad mejora la gráfica a pesar de que es menos precisa a escalas pequeñas, a escalas más grandes puede ser más revelador. es tan funcional que existe un geom dedicado a esta posición `geom_jitter()`

### Ejercicios:
```{r ajuste9, warning=FALSE}
...
quiz(caption = "Responde",
  question_text(
  "1 - ¿Cuál es el problema con esta gráfica? grafica puntos por encima porque hay múltiples observaciones por encima ¿Cómo se puede mejorar?",
  answer(NULL, correct = TRUE),
  incorrect = "Lo estaremos revisando! Gracias!",
  try_again_button = "Modifica tu respuesta",
  allow_retry = TRUE
...
question_text(
  "2 - ¿Qué parámetros de geom_jitter controla la cantidad de vibración? `?geom_jitter` width y height'?",
  answer(NULL, correct = TRUE),
  incorrect = "Lo estaremos revisando! Gracias!",
  try_again_button = "Modifica tu respuesta",
  allow_retry = TRUE
...
question_text(
  "3 - ¿Cuál es la posición por defecto de geom_boxplot?",
  answer(NULL, correct = TRUE),
  incorrect = "Lo estaremos revisando! Gracias!",
  try_again_button = "Modifica tu respuesta",
  allow_retry = TRUE
...
```{r ajuste10, warning=FALSE}
ggplot(data = mpg, mapping = aes(x = cty, y = hwy)) +
  geom_jitter()
```

```{r ajuste11, warning=FALSE}
ggplot(data = mpg, mapping = aes(x = cty, y = hwy)) +
  geom_count()
```

Crea una visualización para demostrarla R = "dodge2, no cambia la posición vertical pero sí cambia la posición  horizontal para evitar que se superponga

```{r ajuste12, warning=FALSE}
