---
title: "Ejercicios de Modelación"
...
sims <- map_df(1:12, ___)

ggplot(sims, aes(x = ___, y = ___)) +
  geom_point() +
...
simdf_norm <- map_df(1:12, ____)

ggplot(simdf_norm, aes(x = ___, y = ___)) +
  geom_point() +
  geom_smooth(method = "lm", colour = "red") +
  facet_wrap(~.id, ncol = 4)
```
...
tibble(
  ...
  normal = dnorm(___),
  student_t = dt(___, df = 2)
) %>%
  pivot_longer(-___, names_to="distribution", values_to="density") %>%
  ggplot(aes(x = ___, y = ____, colour = ____)) +
  geom_line()
```
...
measure_distance <- function(mod, data) {
  diff <- data$y - make_prediction(___, ___)
  mean(abs(____))
}
```
...
```{r ex_10, echo=TRUE}
best <- optim(c(0, 0), ___, data = sim1a)
best$par
```
...
best <- optim(c(0, 0), ____, data = sim1a)
best$par
```
...
```{r ex_14, echo=TRUE}
best3a <- optim(c(0, 0, 0), ____, data = ___)
best3a$par
```

```{r ex_15, echo=TRUE}
best3b <- optim(c(0, 0, 1), ___, data = ___)
best3b$par
```

```{r ex_16, echo=TRUE}
best3c <- optim(c(0, 0, 5), ____, data = ___)
best3c$par
```
...
```{r ex_17, echo=TRUE}
sim1_loess <- loess(___ ~ ____, data = ____)
sim1_lm <- lm(___ ~ ___, data = ____)

grid_loess <- ____ %>%
  add_predictions(____)

sim1 <- ___ %>%
  add_residuals(___) %>%
  add_predictions(____) %>%
  add_residuals(____, var = "resid_loess") %>%
  add_predictions(____, var = "pred_loess")
```
...
```{r ex_18, echo=TRUE}
plot_sim1_loess <-
  ggplot(sim1, aes(x = ___, y = ___)) +
  geom_point() +
  geom_line(aes(x = ___, y = ____), data = grid_loess, colour = "red")
plot_sim1_loess
```
...
```{r ex_24, echo=TRUE}
sim1_mod <- lm(y ~ ___, data = ___)

sim1 <- sim1 %>%
  add_residuals(____)

ggplot(___, aes(x = abs(___))) +
...
```{r ex_25, echo=TRUE}
  mod2a <- lm(___ ~ ___ - 1, data = ____)
```

```{r ex_26, echo=TRUE}
mod2 <- lm(___ ~ ___, data = ___)
```
...
```{r ex_28, echo=TRUE}
x3 <- model_matrix(___ ~ ___ * ___, data = ___)
x3
```
...
```{r ex_32, echo=TRUE}
mod1 <- lm(y ~ ___ + __, data = sim3)
mod2 <- lm(y ~ ___ * ___, data = sim3)
```
...
model_matrix_mod1 <- function(.data) {
  mutate(.data,
    ...
    select(`(___)`, x1, x2b, x2c, x2d)
}
```

```{r ex_35, echo=TRUE}
model_matrix_mod1(___)
```
...
```{r ex_42, echo=TRUE}
mod1 <- lm(y ~ ___ + ___, data = ___)
mod2 <- lm(y ~ ___ * ____, data = ___)
```

  * Y agregua los residuos de estos modelos a los datos de `sim4`:
  
```{r ex_43, echo=TRUE}
sim4_mods <- gather_residuals(____, ____, ____)
```

  * Gráficos de frecuencia de ambos residuos:
  
```{r ex_44, echo=TRUE}
  ggplot(___, aes(x = ___, colour = ___)) +
  geom_freqpoly(binwidth = 0.5) +
  geom_rug()
```
...
```{r ex_45, echo=TRUE}
ggplot(_____, aes(x = abs(____), colour = ___)) +
  geom_freqpoly(binwidth = 0.5) +
  geom_rug()
```
