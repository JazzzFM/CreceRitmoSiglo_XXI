...
Block de dibujo visoespacial, por otro lado, te ayuda a guardar información visual utilizando imágenes
...
**La memoria no es  como una biblioteca, se reconstruye**.
...
La memoria de procedimiento se demuestra con tus acciones y no envuelve directamente reportar el contenido de la memoria.

Como por ejemplo caminar, y cualquier cosa que puedes hacer sin tener la conciencia una explicación de cómo las hiciste como encontrar la forma de llegar a tu casa sin poder decir unas instrucciones precisas.
...
Aquí te explico como se guarda la información en el cerebro basado en principios neurocientíficos.
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "70%", fig.align="center"}
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "50%", fig.align="center"}
knitr::include_graphics("images/19.jpg")
```
...
El resto de este curso estará dedicado en mitigar el olvido en un contexto académica aprenderemos como estudiar espaciadamente en el tiempo para interrumpir la inclinación de la curva de olvido, después discutiremos como profundizar el entendimiento ya que es esencial para el aprendizaje y finalmente discutiremos cómo traer información a la mente desde la memoria puede estimular el proceso de re consolidación y fortalecer el aprendizaje.
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "70%", fig.align="center"}
knitr::include_graphics("images/21.jpg")
```
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "90%", fig.align="center"}
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "50%", fig.align="center"}
knitr::include_graphics("images/26.jpg")
```
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "70%", fig.align="center"}
knitr::include_graphics("images/pensar.jpg")
```
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "70%", fig.align="center"}
knitr::include_graphics("images/27.jpg")
```
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "70%", fig.align="center"}
knitr::include_graphics("images/28.jpg")
```
...
Por Ejemplo: Tomemos a la escasez como una idea abstracta, las clases puede ser definidas como sigue, así como más raro algo es más alto es su valor. Esta descripción contiene muchos términos vagos como raro y valor, entonces ponemos un ejemplo concreto para ilustrar la idea.
...
La **codificación dual** es un proceso de combinar materiales verbales con materiales visuales, las imágenes usualmente son mejor recordadas que las palabras, la teoría de la codificación dual es la idea de que cuando combinamos información en forma de texto y de forma visual.

Nuestro aprendizaje es mejorado porque nosotros procesamos la información visual y verbal mediante diferentes canales.
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "50%", fig.align="center"}
knitr::include_graphics("images/30.jpg")
```
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "50%", fig.align="center"}
knitr::include_graphics("images/31.jpg")
```
...
**Por ejemplo**: Poner las etiquetas en un diagrama es importante, eso es, si las etiquetas no son puestas convenientemente cerca de los objetos que son etiquetados pueden crear una carga cognitiva indeseable y potencialmente impedir el aprendizaje. 
...
La práctica de la recuperación involucra reconstruir algo que has aprendido en el pasado desde la memoria y pensarla en este momento.
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "50%", fig.align="center"}
knitr::include_graphics("images/34.jpg")
```
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "50%", fig.align="center"}
knitr::include_graphics("images/33.png")
```
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "50%", fig.align="center"}
knitr::include_graphics("images/35.jpg")
```
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "50%", fig.align="center"}
knitr::include_graphics("images/36.jpg")
```
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "50%", fig.align="center"}
knitr::include_graphics("images/37.png")
```
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "50%", fig.align="center"}
knitr::include_graphics("images/38.jpg")
```
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "50%", fig.align="center"}
knitr::include_graphics("images/39.jpg")
```
...
```{r fig.margin = TRUE, echo = FALSE, fig.width=3, out.width = "50%", fig.align="center"}
knitr::include_graphics("images/40.jpg")
```
...
La práctica especial es exactamente lo opuesto atiborrarse, cuando te atiborras estudias por periodos prolongados e intensos en lapsos cercanos al examen cuando espacías tu aprendizaje tomas exactamente esa cantidad de tiempo en estudio hilo esparces a lo largo de un período mucho más grande. Hacerlo de esta forma, es exacta cantidad de tiempo en estudios producirá un aprendizaje en el largo plazo.
  ...
  * Por ejemplo: 5 horas esparcidas en dos semanas es mucho mejor que 5 horas antes del examen, pero requiere requieres  planear  anticipadamente, no puedes decidir espaciar tu estudio en el último minuto.
...
Revisa la información de cada clase pero no inmediatamente después de la clase, una buena forma de hacer esto es reservar tiempo al día después de tu clase. 
  ...
  * Por ejemplo: Si para una materia tienes clases lunes miércoles y viernes entonces aparta un momento para revisar el contenido de la clase martes jueves y sábado para cada una de esas clases.
...
La deprivación del sueño puede causar impedimentos en la atención, en la resolución de problemas, y en tomar decisiones (Smith, Robinson & Segal, 2016) lo que es particularmente importante se entenderá que aún una deprivación pequeña del sueño puede causar estos efectos, algunos estudios muestran que el riesgo en la salud y en los impedimentos cognitivos incrementan si pierdes de una a dos horas de sueño cada noche. 

En otras palabras si duermes de 6 a 7 horas todas las noches su funcionalidad cognitiva incluyendo el aprendizaje es probable que sufra una mejora el desempeño especialmente para entender información y resolver problemas.

### Desarrollo del entendimiento.

  **Elaboración**: Haz preguntas de cómo y por qué  las cosas funcionan y luego produce las respuestas a estas preguntas.
...
Hasta ahora te he sugerido usar la elaboración interrogativa para estudiar el contenido de tu clase, al principio, definitivamente puedes utilizar el material de tu clase para ayudarte a llenar esos vacíos cuando estás elaborando. Sin embargo, idealmente, deberías trabajar para describir y explicar las ideas que estás aprendiendo con tus propias palabras sin el material de tu clase enfrente tuyo.
  ...
  * **Ejemplos concretos**: Cuando estás estudiando intenta pensar en cómo puedes convertir ideas que estás aprendiendo en ejemplos concretos, haciéndo un enlace entre la idea que estás estudiando y un ejemplo concreto vivido puede ayudar a que la lección se pegue mucho mejor. 
...
Los maestros pueden destruir la práctica de estacionamiento a sus estudiantes de dos maneras: Al crear oportunidades para revisitar la información a lo largo del curso (espaciamiento) o dentro de cada lección (entrelazado) ayudando a los alumnos a qué creen su propia  calendarizacion de estudio efectivo.
...
¿Cómo implementar la práctica de recuperación dentro del salón? La clave es asegurar que los estudiantes están trayendo información a su mente.
