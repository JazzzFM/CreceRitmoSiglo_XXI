...
"
Esta sección se mostrará cómo utilizar la visualización y la transformación 
para explorar sus datos de manera sistemática, una tarea que los estadísticos
denominan análisis exploratorio de datos o **EDA**
...
### Requisitos previos
"
 Combinaremos lo que hemos aprendido sobre `dplyr` y `ggplot2`
 Para hacer preguntas de forma interactiva, responderlas con datos y
 luego hacer nuevas preguntas.
"
...
"Mucho mejor una respuesta aproximada a la pregunta correcta,
que a menudo es vaga, que una respuesta exacta a la pregunta incorrecta,
que siempre se puede hacer precisos". - John Tukey

"
El objetivo durante **EDA** es desarrollar una comprensión de nuestros datos.

La forma más fácil de hacer esto es usar preguntas ...

Cuando hace una pregunta ...

**EDA** es fundamentalmente un proceso creativo ...

Y como la mayoría de los procesos creativos ...

Por otro lado, cada nueva pregunta que tu le solicitas,
le expondrá un nuevo aspecto de nuestros datos y aumentará
su oportunidad de hacer un descubrimiento.
...
Explicarémos qué son las variaciones y la covariación,
* Una variable es una cantidad, calidad o propiedad que puede medir. 
* Una observación**: es un conjunto de mediciones 

Los datos tabulares son un conjunto de valores,
cada uno asociado con una variable y una observación.

Hasta ahora, todos los datos que has visto ha sido ordenado. 
En la vida real, la mayoría de los datos no están ordenados ...
"
...
Puedes ver la variación fácilmente en la vida real;
Si mide alguna variable continua dos veces, obtendrá dos resultados diferentes.

Cada variable tiene su propio patrón de variación ...

La mejor manera de entender ese patrón es visualizar
...
Una variable es categórica si solo puede tomar uno de un pequeño
conjunto de valores.

En R, las variables categóricas generalmente se guardan como
factores o vectores de caracteres.
...
"
Hay algunos desafíos con este tipo de gráfico, a los que volveremos
al visualizar una variable categórica y una continua.
...
A continuación, hemos elaborado una lista de los tipos de información 
más útiles que encontrará en sus gráficos,

La clave para hacer buenas preguntas
de seguimiento será confiar en tu curiosidad
Así como en tu escepticismo (¿cómo podría ser engañoso?).
"

### Valores típicos

"
Tanto en los gráficos de barras como en los histogramas,
...
Para convertir esta información en preguntas útiles, busque algo inesperado:
  ...
  *   ¿Puedes ver algún patrón inusual? ¿Qué podría explicarlos?
  ...
  Como ejemplo, el histograma a continuación sugiere:
...
"
Para comprender los subgrupos, preguntate:
...
Sin embargo, si tienen un efecto sustancial en sus resultados, no debe descartarlos 
...
Pero tal vez eso se deba a que los polígonos de frecuencia
  son un poco difíciles de interpretar;

Otra alternativa para mostrar la distribución de una variable
continua desglosada por una variable categórica es la gráfica de caja. 

Una caja que se extiende desde el percentil 25 de la distribución 
...
Puntos visuales que muestran observaciones que caen más de 1.5 veces 
el **IQR** desde cualquier borde de la caja.

Una línea (o bigote) que se extiende desde cada extremo de la caja y
va al punto no atípico más lejano de la distribución
"
...
Una forma de demostrarlo es hacer que el ancho de la gráfica de caja 
sea proporcional al número de puntos con `varwidth = TRUE`.
